function initPongGame() {
    ...
    var user = {
        x: 0, y: canvas.height / 2 - 100 / 2, width: 20, height: 100, color: "WHITE", score: 0
    }

    // Create the com paddle Object
    var com = {
        x: canvas.width - 20, y: canvas.height / 2 - 100 / 2, width: 20, height: 100, color: "WHITE", score: 0
    }

    // Create the ball Object
    var ball = {
        x: canvas.width / 2, y: canvas.height / 2, radius: 10, speed: 5, velocityX: 5, velocityY: 5, color: "WHITE"
    }
    ...
    var gameInterval = setInterval(game, 1000 / framePerSecond);

    // Ajouter l'écouteur d'événement
    canvas.addEventListener("mousemove", movePaddle);

    // Fonction pour arrêter le jeu et supprimer les écouteurs d'événements
    function stopGame() {
        clearInterval(gameInterval); // Arrêter la boucle du jeu
        canvas.removeEventListener("mousemove", movePaddle); // Supprimer l'écouteur d'événement
    }

    // Expose stopGame pour une utilisation externe
    return {
        stopGame: stopGame
    };
}
